...
async def compileGarage(username):
    ...
    with Image.new('RGBA', (width, height)) as img:
        ...
        garage = numpy.reshape(profile["data"]['garage'],
                               (ceil(len(profile["data"]['garage']) / 15), 15))
...
bot = commands.Bot(
    command_prefix=str(data["botPrefix"]),
    activity=discord.Game(name=f"{str(data['botPrefix'])}garage [username]"))
...
async def find(ctx, carName: str):
    try:
        ...
        for x in carDict:
            await ctx.send(
                f'__**{x}:**__ https://www.nitrotype.com/cars/{carDict[x]}_large_1.png'
            )
    except:
